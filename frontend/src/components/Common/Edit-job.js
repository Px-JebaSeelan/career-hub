...
const EditJob = () => {
    ...
    const onSubmit = (e) => {
        ...
        axios.post(API_URL + '/job/job/edit', newJob)
            .then(res => {
    ...
    return (
        <div className="container" style={{ padding: '3rem 1rem', display: 'flex', justifyContent: 'center' }}>
            <div className="glass" style={{ width: '100%', maxWidth: '800px', padding: '2.5rem', borderRadius: 'var(--radius-xl)' }}>
                <h2 style={{ textAlign: 'center', marginBottom: '2rem' }}>Edit Job Details</h2>

                <form onSubmit={onSubmit} style={{ display: 'grid', gap: '1.5rem' }}>

                    <div className="form-group">
                        <label>Job Title (Confirm)</label>
                        <input
                            type="text"
                            name="title"
                            ...
                            placeholder="Enter title of job to edit"
                            required
                        />
                    </div>
